...
public class SendBinarySubmitSmRequestCommandTest {

    // Object under test
    private SendBinarySubmitSmRequestCommand objectUnderTest;
    ...
    public void setUp() throws Exception {
        ...
        objectUnderTest = new SendBinarySubmitSmRequestCommand(mockConnectionHandler,
                                                               SOURCE_ADDRESS,
                                                               DESTINATION_ADDRESS,
    ...
    @Test
    public void shouldThrowSendSmppExceptionWheRateExeceeded() throws Exception {
        final WindowFuture<Integer, PduRequest, PduResponse> defaultResponse = new TestWindowFuture(true,
                                                                                                    true,
                                                                                                    pduRequest);
        when(mockConnectionHandler.getSmppSession()).thenReturn(mockSmppSession);
        ...
        when(mockAddressFormatter.formatDestinationAddress(DESTINATION_ADDRESS)).thenReturn(pduRequest.getDestAddress());
        when(mockConnectionHandler.getRateLimitingTokenBucket()).thenReturn(mockTokenBucket);
        when(mockTokenBucket.acquirePermit(anyLong(), anyObject())).thenReturn(false);
    ...
    @Test
    public void shouldAddMessageAsOptionalParam() throws Exception {
        objectUnderTest = new SendBinarySubmitSmRequestCommand(mockConnectionHandler,
                                                               SOURCE_ADDRESS,
                                                               DESTINATION_ADDRESS,
                                                               messageText.getBytes(),
                                                               mockAddressFormatter);
        final SubmitSm result = objectUnderTest.buildSubmitSmRequest();
        assertThat(result.getShortMessageLength(), is(0));
        assertThat(result.getOptionalParameter(SmppConstants.TAG_MESSAGE_PAYLOAD).getValue(),
                   is(messageText.getBytes()));
    }
