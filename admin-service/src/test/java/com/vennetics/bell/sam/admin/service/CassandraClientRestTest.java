...
/**
 * CassandraClient Rest endpoint tests
 */
...
public class CassandraClientRestTest {

    @ClassRule
    public static final RestDocumentation REST_DOCUMENTATION = new RestDocumentation(
                    "build/generated-snippets");
    private static final Logger logger = LoggerFactory.getLogger(CassandraClientRestTest.class);
    ...
    public void testCrudClient() throws Exception {
        CassandraClient testCassandraClient = TestUtils.createDummyCassClient();

        // Create
        URI uri = new URI("http://127.0.0.1:" + port + "/" + CassandraClient.TABLE_NAME);
        mockMvc.perform(post(uri).contentType(contentType)
                .content(TestUtils.convertObjectToJsonBytes(testCassandraClient))
                .accept(contentType))
                .andDo(print())
                .andExpect(status().isCreated())
                .andExpect(content().contentType(contentType))
                .andExpect(jsonPath("$.applicationId").value(APPLICATION_ID_1.toString()))
                ...
                .andDo(document("clients",
                        preprocessRequest(prettyPrint()),
                        preprocessResponse(prettyPrint()),
                        requestFields(fieldWithPath("clientId").description("The ID of this Client."),
                                fieldWithPath("applicationId").description("The Application ID related to this Client."),
                        ...
                        responseFields(fieldWithPath("applicationId").description("The Application ID related to this Client."),
                                fieldWithPath("createdTimestamp").description("The date and time this record was created."),
                                fieldWithPath("lastModifiedTimestamp").description("The date and time this record was last modified."),
        ...
        // Read
        mockMvc.perform(get(uri + "/" + testCassandraClient.getClientId()).contentType(contentType)
                                                                          .content(TestUtils.convertObjectToJsonBytes(testCassandraClient))
                                                                          .accept(contentType))
                .andDo(print())
        ...
        // Update
        testCassandraClient.setApplicationId(APPLICATION_ID_2);
        mockMvc.perform(put(uri + "/" + testCassandraClient.getClientId()).contentType(contentType)
                                                                          .content(TestUtils.convertObjectToJsonBytes(testCassandraClient))
                                                                          .accept(contentType))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentType(contentType))
                .andExpect(jsonPath("$.applicationId").value(APPLICATION_ID_2.toString()))
                .andExpect(jsonPath("$.createdTimestamp").value(dateFormat.format(TestUtils.CREATION_DATE)))
                .andExpect(jsonPath("$.lastModifiedTimestamp").value(dateFormat.format(TestUtils.MODIFIED_DATE)))
                .andDo(document("clients",
                        preprocessRequest(prettyPrint()),
        ...
        // Delete
        mockMvc.perform(delete(uri + "/" + testCassandraClient.getClientId()).contentType(contentType)
                                                                             .content(TestUtils.convertObjectToJsonBytes(testCassandraClient))
                                                                             .accept(contentType))
                .andDo(print())
