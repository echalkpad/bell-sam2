...
/**
 * CassandraProtocolType Rest endpoint tests
 */
...
public class CassandraProtocolTypeRestTest {

    @ClassRule
    public static final RestDocumentation REST_DOCUMENTATION = new RestDocumentation(
                    "build/generated-snippets");
    private static final Logger logger = LoggerFactory.getLogger(CassandraProtocolTypeRestTest.class);
    ...
    @Test
    public void testCrudProtocolType() throws Exception {
        CassandraProtocolType testCassandraProtocolType = TestUtils.createDummyCassProtocolType();

        // Create
        URI uri = new URI("http://127.0.0.1:" + port + "/" + CassandraProtocolType.TABLE_NAME);
        mockMvc.perform(post(uri).contentType(contentType)
                .content(TestUtils.convertObjectToJsonBytes(testCassandraProtocolType))
                .accept(contentType))
                .andDo(print())
                ...
                .andDo(document("protocol_types",
                        preprocessRequest(prettyPrint()),
                        preprocessResponse(prettyPrint()),
                        requestFields(fieldWithPath("protocolTypeId").description("The ID of this Protocol Type."),
                                fieldWithPath("description").description("The description for this Protocol Type."),
                                fieldWithPath("serviceTypeIds").description("A Set of ServiceTypeIds associated with this Protocol Type.")),
                        responseFields(fieldWithPath("description").description("The description for this Protocol Type."),
                                fieldWithPath("serviceTypeIds").description("A Set of ServiceTypeIds associated with this Protocol Type."),
                                fieldWithPath("_links").description("The returned links."))
                ));

        // Read
        mockMvc.perform(get(uri + "/" + testCassandraProtocolType.getProtocolTypeId()).contentType(contentType)
                                                                                      .content(TestUtils.convertObjectToJsonBytes(testCassandraProtocolType))
                                                                                      .accept(contentType))
                .andDo(print())
        ...
        // Update
        testCassandraProtocolType.setDescription(TestUtils.UPDATED_DESCRIPTION);
        mockMvc.perform(put(uri + "/" + testCassandraProtocolType.getProtocolTypeId()).contentType(contentType)
                                                                                      .content(TestUtils.convertObjectToJsonBytes(testCassandraProtocolType))
                                                                                      .accept(contentType))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentType(contentType))
                .andExpect(jsonPath("$.description").value(TestUtils.UPDATED_DESCRIPTION))
                .andExpect(jsonPath("$.serviceTypeIds").isArray())
                .andExpect(jsonPath("$.serviceTypeIds[0]").value(SERVICE_IDS.toArray()[0].toString()))
                .andDo(document("protocol_types",
                        preprocessRequest(prettyPrint()),
        ...
        // Delete
        mockMvc.perform(delete(uri + "/" + testCassandraProtocolType.getProtocolTypeId()).contentType(contentType)
                                                                                         .content(TestUtils.convertObjectToJsonBytes(testCassandraProtocolType))
                                                                                         .accept(contentType))
                .andDo(print())
